...
dtm_clean <- function(dtm, meta = metadata, sparsity = .95, idf = TRUE){
    rownames(dtm) <- meta$shortname
    dtm %>% 
        removeSparseTerms(.95)  %>%
        weightTfIdf(normalize = T) %>%
        return()
}
dtm3 <- dtm_clean(dtm)
...
dtm3_dissim <- dissim(dtm3)
```

#  Heirarchical clustering
```{r MultiDimensional Scaling, fig.height=6}
plot(agnes(dtm3_dissim, method = "ward"), which.plots = 2)
...
```{r}
dtm4 <- dtm_clean(dtm, sparsity = .1, idf = TRUE)
